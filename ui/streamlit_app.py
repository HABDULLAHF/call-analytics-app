...
with st.sidebar:
    ...
    analysis_mode = st.radio(
        "Analysis Mode",
        options=["Analysis", "AI (OpenAI)"],
        index=0,
        help="AI mode calls /insights_ai and then renders the same analytics + an AI insights section."
    )
    ...
    target_mode = colu1.radio(
        ...
        options=["API (/upload)"],
        index=0,
        help="Uploads to FastAPI's /upload, then triggers /reload."
    )
...
if main or fetch_btn:
    ...
    # Support both new and legacy keys from API
    contacts_list = contacts_data.get("contacts_overview")
    ...
    # Choose endpoint:
    # - AI mode: /insights_ai → use returned simple_payload for charts; add insights_md at the end.
    # - Simple mode: /stats
    use_ai = (analysis_mode.startswith("AI"))
    insights_md = None
    if use_ai:
        ai_resp = fetch_json(f"{API_URL}/insights_ai", params=params, timeout=120)
        if ai_resp and isinstance(ai_resp.get("simple_payload"), dict):
            stats_data = ai_resp["simple_payload"]
            insights_md = ai_resp.get("insights_md")
            if ai_resp.get("error"):
                st.warning(f"AI/Analytics note: {ai_resp['error']}")
        else:
            st.info("AI endpoint not available — falling back to Simple.")
    ...
    if not calls_df.empty:
        ...
        # ======== 📅 Per-Date Summary (from API if present; else compute) ========
        st.markdown("### 📅 Per-Date Summary")
        if percent is not None:
            st.metric("Contact Percentage (of main)", f"{float(percent):.2f}%")

        per_date = stats_data.get("per_date_summary")
        if isinstance(per_date, list) and len(per_date) > 0:
            ...
            if "date" in calls_df.columns:
                ...
                per_date_df = pd.DataFrame(per_date_rows).sort_values("Date") if per_date_rows else pd.DataFrame()
            else:
        ...
        if not per_date_df.empty:
            ...
            st.caption("No per-date summary available.")

        # ======== Distribution charts ========
        st.markdown("### Distribution by Direction & Call Type")

        # Prefer API aggregates if present
        dir_counts_api = stats_data.get("direction_counts")
        type_counts_api = stats_data.get("call_type_counts")

        if isinstance(dir_counts_api, list) and len(dir_counts_api) > 0:
            dir_counts = pd.DataFrame(dir_counts_api)
            dir_counts.columns = [str(c).lower() for c in dir_counts.columns]
        else:
            dir_counts = counts_df_from_api_or_local(stats_data, calls_df, "direction_counts", "direction")

        if isinstance(type_counts_api, list) and len(type_counts_api) > 0:
            type_counts = pd.DataFrame(type_counts_api)
            type_counts.columns = [str(c).lower() for c in type_counts.columns]
        else:
        ...
        if use_ai:
            ...
            if insights_md:
                ...
                st.caption("AI insights not available (model disabled or endpoint error).")
