...
#         "Analysis Mode",
#         options=["Simple (local pandas on server)", "AI"],
#         index=0,
#         help="AI calls /stats_ai (falls back to /stats if unavailable)."
#     )
...
#     # NEW: toggle AI vs Simple endpoint (+ fallback)
#     use_ai = (analysis_mode == "AI")
#     stats_endpoint = "/stats_ai" if use_ai else "/stats"
#     stats_data = fetch_json(f"{API_URL}{stats_endpoint}", params=params, timeout=120)
#     if not stats_data and use_ai:
...
with st.sidebar:
    ...
    analysis_mode = st.radio(
        "Analysis Mode",
        options=["Simple (server pandas)", "AI (OpenAI)"],
        index=0,
        help="AI calls /stats_ai (falls back to /stats if unavailable)."
    )
...
if main or fetch_btn:
    ...
    # Choose endpoint based on toggle (+ fallback)
    ...
    if "error" in stats_data and stats_data["error"]:
        st.warning(f"AI/Analytics note: {stats_data['error']}")
